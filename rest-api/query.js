...
var getBlockByNumber = async function (peer, channelName, blockNumber, username, orgName) {
	try {
...
var getTransactionByID = async function (peer, channelName, trxnID, username, orgName) {
	try {
		// first setup the client for this org
		var client = await helper.getClientForOrg(orgName, username);
		logger.debug('Successfully got the fabric client for the organization "%s"', orgName);
...
var getBlockByHash = async function (peer, channelName, hash, username, orgName) {
	try {
		// first setup the client for this org
		var client = await helper.getClientForOrg(orgName, username);
		logger.debug('Successfully got the fabric client for the organization "%s"', orgName);
