	...
	try {
		...
		for (const i in proposalResponses) {
			...
			if (proposalResponses[i] instanceof Error) {
				...
				error_message = util.format('##### invokeChaincode - received unsuccessful proposal response :: %s', proposalResponses[i].toString());
				logger.error(error_message);
			} else if (proposalResponses[i].response && proposalResponses[i].response.status === 200) {
				logger.info('##### invokeChaincode - received successful proposal response');
			} else {
				successfulResponses = false;
				error_message = util.format('##### invokeChaincode -  failed for an unknown reason %j', proposalResponses[i]);
				logger.error(error_message);
			}
		}
